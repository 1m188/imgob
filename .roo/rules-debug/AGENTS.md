...
- 图片不显示但 loading 状态正确：检查 CSS — `.image-canvas__img` 使用 `position: absolute; inset: 0; width: 100%; height: 100%`。如果父容器 `.image-canvas` 没有 `height`，图片会是 0 像素。确认 `#app` 和 `body/html` 的 `height: 100%` 链完整，且 `main.css` 已被导入（`src/main.js` 中有 `import './assets/main.css'`）。
...
- GitHub Pages 部署后页面空白/资源 404：检查 `docs/index.html` 中的资源路径是否以 `/imgob/` 开头。`vite build` 的 `base` 仅在 `NODE_ENV=production` 时生效，本地 `npm run build` 不会注入 `/imgob/` 前缀。
